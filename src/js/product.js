...
register('product', {
  async onLoad() {
    ...
    document.querySelectorAll(selectors.altImages).forEach(altImage => {
      altImage.addEventListener('click', this.onAltImageClick.bind(this));
    });
  },
  ...
  onAltImageClick(event){
    document.querySelectorAll(selectors.altImages).forEach(altImage => altImage.parentElement.classList.remove('active'));
    event.target.parentElement.classList.add('active');
    const newSrc = event.target.dataset.altImage;
    document.querySelector(selectors.featuredImage).setAttribute('src', newSrc);
  },
  ...
  renderSubmitButton(variant) {
    ...
    if (!variant) {
      ...
      submitButtonText.innerText = 'Product unavailable';
